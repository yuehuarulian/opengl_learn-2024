...
void Sphere::init(int prec)
{
    ...
    for (int i = 0; i < numVertices; i++) // std::vector::push_back()
        vertices.push_back(glm::vec3());

    for (int i = 0; i < numVertices; i++) // inserts new element at
        texCoords.push_back(glm::vec2());

    for (int i = 0; i < numVertices; i++) // the end of a vector and
        normals.push_back(glm::vec3());

    for (int i = 0; i < numIndices; i++) // increases the vector size by 1
        indices.push_back(0);
