...
int main()
{
    ...
    // 创建着色器对象
    Shader lightingShader("./shaderprogram/class10_vertexshader", "./shaderprogram/class10_fragmentshader");

    Shader lightingShader_c("shaderprogram/homework2_2.vertexshader", "shaderprogram/homework2_2.fragmentshader");

    glEnable(GL_STENCIL_TEST);
    glStencilFunc(GL_NOTEQUAL, 1, 0xFF); // glStencilFunc(GLenum func, GLint ref, GLuint mask)
    ...
    while (glfwWindowShouldClose(window) == 0 && glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS) // 窗口没有关闭，esc键没有按下
    {
        ...
        for (unsigned int i = 0; i < 4; i++)
        {
            ...
            lightingShader.setVec3("pointlights[" + num + "].diffuse", 0.8f, 0.8f, 0.8f);
            lightingShader.setVec3("pointlights[" + num + "].specular", 1.0f, 1.0f, 1.0f);
            lightingShader.setFloat("pointlights[" + num + "].constant", 1.0f);
            lightingShader.setFloat("pointlights[" + num + "].linear", 0.09);
            lightingShader.setFloat("pointlights[" + num + "].quadratic", 0.032);
        }
