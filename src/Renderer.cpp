...
color Renderer::trace(const Ray &ray, const shared_ptr<Scene> &scene, int depth)
{
	...
	if (type == MatType::GLASS)
	{
		...
		Ray reflect_ray(rec.p + reflect_dir * vec3(0.001), reflect_dir); // 创建反射光线
		vec3 reflect_color = trace(reflect_ray, scene, depth + 1);		 // 追踪反射光线

		// 折射光线
		vec3 refract_dir = glm::refract(ray.dir, normal, eta);
		Ray refract_ray(rec.p + refract_dir * vec3(0.001), refract_dir); // 创建折射光线
		vec3 refract_color = trace(refract_ray, scene, depth + 1);		 // 追踪折射光线
	...
	else if (type == MatType::DIFFUSE)
	{
		...
		for (const auto &light : scene->lights)
		{
			...
			vec3 light_dir = normalize(light_ptr->center - rec.p); // 光源方向
			...
			vec3 reflect_dir = reflect(-light_dir, normal);												 // 反射光线方向
			float spec_intensity = std::max(dot(view_dir, reflect_dir), 0.0f);							 // 计算反射强度
			spec_intensity = pow(spec_intensity, 32);													 // 反射光的光滑度（shininess）
			vec3 specular = spec_intensity * light_ptr->mat_ptr->albedo / (dis_to_light * dis_to_light); // 镜面反射光照（通常为白色）

			// Blinn-Phong高光
			vec3 half_dir = normalize(light_dir + view_dir);								  // 半程向量
			float blinn_spec_intensity = pow(glm::max(glm::dot(normal, half_dir), 0.0f), 32); // 高光的强度
			vec3 blinn_specular = blinn_spec_intensity * light_ptr->mat_ptr->albedo / (dis_to_light * dis_to_light);

			// 阴影检测：从交点到光源发射射线，检查是否被遮挡
			Ray shadow_ray(rec.p, light_dir); // 从物体表面偏移一点，避免自交
